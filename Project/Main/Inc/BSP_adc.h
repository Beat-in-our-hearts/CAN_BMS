 ...
 *                      【函数说明】 BSP_ADC_Base_Init       开启时钟；初始IO；设定循环采集通道；校准
 *                                  BSP_DMA_ADC_Tx_Init     DMA设置
 *                                  BSP_ADC_NVIC_Config     设置中断优先级
 *                                  BSP_ADC_Init            主函数中依次调用上述函数
 *                                  BSP_Get_Float_ADC       将缓冲区adc整型数值转化成浮点类型的电压电流信息
 ************************************************************************************************************************************************/ 
#ifndef __BSP_ADC_H
#define __BSP_ADC_H
...
//// void BSP_Set_ADC_Freq(void);     // 设置定时器TIM3 TRGO 触发ADC采集的频率
void BSP_ADC_NVIC_Config(void);     // 设置所需的中断函数优先级
void BSP_ADC_Init(void);            // 设置ADC功能：端口初始化、ADC初始化、DMA初始化////定时器初始化

// 数组结构 前n-1为电流 最后1位电压
////void BSP_Get_Float_ADC(uint16_t *buf, float *res);  // ADC转实际电压
